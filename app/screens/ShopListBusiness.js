...
export default function ShopListBusiness(props) {
  ...
  const navfunctions=() => { 
    if(title!==''&&desc!=="" && category!=='' && image!==null)
    {
      props.navigation.navigate("ShopDetailsScreen",{buisenessinfo:{name:title,description:desc,category:category,active:true,
        business_PrimaryImage:image}})
    }
   }
