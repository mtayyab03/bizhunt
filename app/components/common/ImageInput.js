...
export default function ImageInput({imageUri,price2,title2,getdetails,ls,rmp,iteminfo}) {
    ...
    const handlepress=()=>{
        if(!imageUri) pickImage();
            ...
            {text:'Yes',onPress:()=>{rmp({group:iteminfo?.group,url:imageUri})}},
            {text:'No'}
        ])
    ...
    const pickImage = async () => {
        ...
        if (!result.canceled) {
           ...
           setimagebase64(`data:${result.assets[0].type};base64,${result.assets[0].base64}`)    
            }
      };
  return (
    <>
        ...
        <View style={{flex: 1,
          ...
          {/* modal title & price */}
           ...
           <TouchableOpacity activeOpacity={0.7} onPress={() => {
            setModalVisible(!modalVisible)
          getdetails(imagebase64,lks,pr,ti)
          ls(iteminfo?.group) 
          }} 
